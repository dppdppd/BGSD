  ...
  {#if libMenu}
    ...
    <div class="lib-context-menu" style="left: {libMenu.x}px; top: {libMenu.y}px;">
      {#if libMenu.isRepo}
        ...
        <button class="lib-context-item" data-testid="ctx-export-stl" onclick={() => onexportstl(libMenu!.path)}>Export STLs</button>
      {:else}
        ...
        <button class="lib-context-item" data-testid="ctx-duplicate" onclick={() => { const p = libMenu!.path; libMenu = null; onduplicatefile(p); }}>Duplicate</button>
