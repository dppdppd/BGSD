...
describe("STL export planning", () => {
  ...
  it("rewrites Make calls with an explicit print group selector", () => {
    expect(scadWithPrintGroup(SCAD_WITH_GROUPS, "red")).toContain('Make(data, print_groups = "red");');
    expect(scadWithPrintGroup(SCAD_WITH_GROUPS, false)).toContain("Make(data, print_groups = []);");
    expect(scadWithPrintGroup(SCAD_WITH_GROUPS.replace("Make(data);", 'Make(data, print_groups = "blue");'), false))
      .toContain("Make(data, print_groups = []);");
  });

  it("keeps the legacy print_group override for pre-4.11 BIT designs", () => {
    const legacyScad = SCAD_WITH_GROUPS
      .replace("boardgame_insert_toolkit_lib.4.11.0.scad", "boardgame_insert_toolkit_lib.4.9.1.scad")
      .replace("G_PRINT_GROUPS", "G_PRINT_GROUP");

    expect(scadWithPrintGroup(legacyScad, "red")).toContain('Make(data, print_group = "red");');
    expect(scadWithPrintGroup(legacyScad, false)).toContain("Make(data, print_group = false);");
  });
  ...
  it("plans one combined STL plus one STL per print group", () => {
    ...
    expect(plan.jobs[0].scadText).toContain("Make(data, print_groups = []);");
    expect(plan.jobs[1].scadText).toContain('Make(data, print_groups = "blue");');
  });
