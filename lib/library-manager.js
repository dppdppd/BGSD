 ...
 * Returns { <profileId>: { name, hasUpdate, files: [{path, hasUpdate}] } } per
 * configured profile. Network failures / missing manifests resolve to
...
async function checkLibraryUpdates(workingDir) {
  ...
  for (const [profileId, profile] of Object.entries(profiles)) {
    ...
    result[profileId] = { name: profile.name, hasUpdate, files };
  }
  return result;
}
