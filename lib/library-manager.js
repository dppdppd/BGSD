...
/**
 * Download a repo tarball from codeload.github.com and extract release/*.scad files.
 * Returns array of { localPath, content } for all .scad files under release/.
 * Single HTTP request — no API rate limits, no github.com dependency.
 */
function fetchReleaseTarball(repo, branch, onProgress) {
  ...
  return new Promise((resolve, reject) => {
    https.get(url, { headers: { "User-Agent": "BGSD" } }, (res) => {
      if (res.statusCode !== 200) {
        reject(new Error(`HTTP ${res.statusCode} fetching tarball from ${url}`));
        return;
      }
      const files = [];
      res.pipe(zlib.createGunzip()).pipe(new tar.Parse())
        .on("entry", (entry) => {
          // Tar entries have a repo-branch prefix, e.g. "repo-master/release/lib/foo.scad"
          const parts = entry.path.split("/").slice(1); // strip prefix
          const relPath = parts.join("/");
          if (relPath.startsWith("release/") && relPath.endsWith(".scad")) {
            const localPath = relPath.slice("release/".length);
            let content = "";
            entry.on("data", (c) => (content += c));
            entry.on("end", () => {
              log(`Found ${localPath}`);
              files.push({ localPath, content });
            });
          } else {
            entry.resume(); // drain unneeded entries
          }
        })
...
async function initWorkingDir(workingDir, onProgress) {
  ...
  for (const [profileId, profile] of Object.entries(profiles)) {
    ...
    let files;
    try {
      files = await fetchReleaseTarball(profile.repo, profile.branch, (msg) => log(`[${profile.name}] ${msg}`));
    } catch (err) {
