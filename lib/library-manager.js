...
async function resolveLatestLibraryFile(profileId, onProgress) {
  ...
  const candidates = tree
    ...
    .map((e) => {
      ...
      return { repoPath: e.path, filename, version: filenameVersion(filename) };
    })
    .filter((e) => e.version);
...
async function ensureLibraryFile(profileId, filename, options = {}) {
  ...
  const repoPath = profileLibRepoPath(profile, filename);
...
async function ensureLatestLibrary(profileId, options = {}) {
  const latest = await resolveLatestLibraryFile(profileId, options.onProgress);
      ...
      try {
        ...
        if (fs.existsSync(libDir)) {
          for (const name of fs.readdirSync(libDir)) {
            ...
            const versionFromName = filenameVersion(name);
            let version = versionFromName;
            if (!version) {
              try { version = parseLibVersion(fs.readFileSync(path.join(libDir, name), "utf-8")); } catch { /* ignore */ }
            }
            if (!version) continue;
